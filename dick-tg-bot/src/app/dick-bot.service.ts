...
export class DickBotService {
  ...
  public async measure(@Ctx() ctx: Scenes.SceneContext) {
    ...
    if (todayUserMeasurement) {
      await ctx.telegram.editMessageText(ctx.chat!.id, waitMessage.message_id, undefined, `Замер уже был произведён, результат ${todayUserMeasurement.result}`);
      return;
    }
    ...
    const resultText = await this._chatGptService.simplePrompt(newResultPrompt(userMeasurement.result), 'gpt-4');
    const text = `*Результат: ${userMeasurement.result}*\n\`Средний результат \\(SMA 10\\): ${escapeMarkdown(String(dickUser.avgResult))}\`\n\n_${escapeMarkdown(resultText)}_`;
  ...
  public async overall(@Ctx() ctx: Scenes.SceneContext) {
    ...
    const text = data.map(([ name, result, text ]) => `*${name} \\- ${result}*\n${escapeMarkdown(text)}`).join('\n\n');
